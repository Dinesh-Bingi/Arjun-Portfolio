...
const ImageItem = ({ image }: ImageItemProps) => {
  ...
  // Check if this is the Lost Underworld project and image has a link
  const isLostUnderworldWithLink =
    typeof window !== 'undefined' &&
    window.location.pathname.includes("the-lost-underworld") &&
    !!image.link;

  return (
    <div className="flex flex-col items-center gap-3">
      <div className={`rounded-xl overflow-hidden border border-border/40 shadow-lg w-full aspect-square relative ${isLostUnderworldWithLink ? 'group' : ''}`}>
        {hasImage && !imageError ? (
          isLostUnderworldWithLink ? (
            <a href={image.link} target="_blank" rel="noopener noreferrer" className="block w-full h-full cursor-pointer overflow-hidden">
              <img
                src={image.src}
                alt={image.alt || ""}
                className="w-full h-full object-cover transition-all duration-200 group-hover:scale-110 group-hover:blur-sm"
                onError={() => setImageError(true)}
              />
              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center pointer-events-none">
                <span className="text-white font-medium px-4 py-2 border border-white/50 rounded-md backdrop-blur-sm shadow-md">
            ...
            <img
              src={image.src}
              alt={image.alt || ""}
              className="w-full h-full object-cover transition-transform duration-200 hover:scale-100"
              onError={() => setImageError(true)}
            />
          )
        ) : (
